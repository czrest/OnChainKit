...
export default function Questions({ form }) {
        ...
        <CardContent className="pt-6 space-y-4">
         <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-4">
           <div className="space-y-4">
            <div>
             ...
             <Input
              ...
              value={question.text}
              onChange={(e) =>
               updateQuestionField(form, index, "text", e.target.value)
              }
              placeholder="Enter your question"
              className={questionError?.text ? "border-destructive" : ""}
             />
             {questionError?.text && (
            ...
            <div className="flex items-center space-x-3">
             <Switch
              ...
              onCheckedChange={(checked) =>
               updateQuestionField(form, index, "required", checked)
              }
             />
             <Label
           ...
           <div>
            <FormLabel htmlFor={`type-${question.id}`}>Question Type</FormLabel>
            <Select
             value={question.type}
             onValueChange={(value) =>
              updateQuestionField(form, index, "type", value)
             }
            >
              ...
              {(question.options || []).map((option, optionIndex) => (
               ...
               >
                <Input
                 value={option}
                 onChange={(e) =>
                  updateOption(form, index, optionIndex, e.target.value)
                 }
                ...
                <Button
                 ...
                 variant="ghost"
                 size="icon"
                 onClick={() => removeOption(form, index, optionIndex)}
                >
          ...
          {currentQuestions.length > 1 && (
           <Button
            ...
            variant="ghost"
            size="icon"
            onClick={() => removeQuestionHandler(form, index)}
           >
            <Trash2 className="h-4 w-4 text-destructive" />
           </Button>
          )}
