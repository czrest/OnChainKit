"use client";

import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Coins, Calendar, CheckCircle } from "lucide-react";
import StatusBadge from "../status-badge";

export default function AnsweredSurveysTab({ surveys, userAddress }) {
  if (surveys.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400 mb-4">
          You haven&apos;t answered any surveys yet
        </p>
        <Link href="/surveys">
          <Button className="bg-gradient-to-r from-emerald-500 to-lime-500 hover:from-emerald-600 hover:to-lime-600">
            Browse Surveys
          </Button>
        </Link>
      </div>
    );
  }

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {surveys.map((survey) => {
        const userResponse = survey.responses.find(
          (r) => r.respondent === userAddress
        );
        const rewardAmount = survey.finalized
          ? (Number(survey.reward) / survey.responses.length).toFixed(4)
          : "0";

        return (
          <div
            key={survey.id}
            className="group p-6 bg-gradient-to-br from-white/5 to-white/10 rounded-lg border border-white/10 hover:border-emerald-500/50 transition-all duration-300"
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-xl font-semibold mb-2">{survey.title}</h3>
                <p className="text-gray-400 text-sm line-clamp-2">
                  {survey.description}
                </p>
              </div>
              <StatusBadge survey={survey} />
            </div>

            <div className="flex flex-wrap gap-4 mb-4">
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-emerald-400" />
                <span className="text-sm">Responded</span>
              </div>
              <div className="flex items-center gap-2">
                <Calendar className="w-4 h-4 text-emerald-400" />
                <span className="text-sm">
                  {userResponse
                    ? new Date(userResponse.timestamp).toLocaleDateString()
                    : "N/A"}
                </span>
              </div>
              {survey.finalized && (
                <div className="flex items-center gap-2">
                  <Coins className="w-4 h-4 text-lime-400" />
                  <span className="text-sm font-semibold text-lime-400">
                    {rewardAmount} ETH earned
                  </span>
                </div>
              )}
            </div>

            <Link href={`/surveys/${survey.id}`}>
              <Button
                variant="outline"
                className="w-full border-white/10 hover:border-emerald-500/50"
              >
                View Survey
              </Button>
            </Link>
          </div>
        );
      })}
    </div>
  );
}
